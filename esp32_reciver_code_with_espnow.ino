// esp32 reciver code with espnow 
#include <Arduino.h>
#include <WiFi.h>
#include <WebServer.h>
#include <esp_now.h>
...
// Function to forecast demand based on historical data
...
// Corrected callback function executed when data is received via ESP-NOW
void OnDataRecv(const esp_now_recv_info *recv_info, const uint8_t *incomingData, int len) {
    // Copy the incoming data into the sensorData structure
    memcpy(&sensorData, incomingData, sizeof(sensorData));
    Serial.println("Data received via ESP-NOW!");

    // Update leak predictions
...
void setup() {
    ...
    // Register the callback function (with the correct signature)
    ...
    // Initialize leak prediction array and sensor data structure
    for (int i = 0; i < 3; i++) {
